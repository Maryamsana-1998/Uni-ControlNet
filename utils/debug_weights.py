...
def integrate(local_weights, global_weights, config_path, output_path):
    ...
    for sk in scratch_dict.keys():
        if sk in local_weights and sk in global_weights:
            if not local_weights[sk].equal(global_weights[sk]):
                print(f"Mismatch found in key: {sk}")
                print(f"Local weights shape: {local_weights[sk].shape}, Global weights shape: {global_weights[sk].shape}")
                # Handle the mismatch appropriately here
            target_dict[sk] = local_weights[sk].clone()
        elif 'local_adapter' in sk:
            if sk in local_weights.keys():
                target_dict[sk] = local_weights[sk].clone()
            else:
                print(f"Key {sk} not found in local weights.")
        elif 'global_adapter' in sk:
            if sk in global_weights.keys():
                target_dict[sk] = global_weights[sk].clone()
            else:
                print(f"Key {sk} not found in global weights.")
        else:
