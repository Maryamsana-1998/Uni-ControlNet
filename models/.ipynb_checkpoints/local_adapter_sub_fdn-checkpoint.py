...
class FeatureExtractor(nn.Module):
    def __init__(self, local_channels, inject_channels, dims=2):
        ...
        self.concat_conv = nn.Conv2d(256, 128, kernel_size=3, padding=1)

    def add_uniform_noise(self,tensor, noise_level=0.1):
        """
        Adds uniform noise to the given tensor.

        Args:
            tensor (torch.Tensor): The input tensor to add noise to.
            noise_level (float): The range of the uniform noise [-noise_level, noise_level].

        Returns:
            torch.Tensor: The tensor with added noise.
        """
        noise = torch.rand_like(tensor) * 2 * noise_level - noise_level  # Uniform noise in [-noise_level, noise_level]
        return tensor + noise
    ...
    def forward(self, local_conditions):
        ...
        op = local_conditions[:, :3, :, :]  # First 3 channels
        frame = local_conditions[:, 3:, :, :]  # Last 3 channels

        # Pass each part through the pre_extractor
        local_features_op = self.pre_extractor(op,None)
        local_features_frame = self.pre_extractor(frame,None)
        
        # Add uniform noise to local_features_op
        local_features_op = self.add_uniform_noise(local_features_op, noise_level=0.1)

        # Concatenate the outputs along the channel dimension
        concatenated_features = torch.cat([local_features_op, local_features_frame], dim=1)

        # Pass the concatenated features through the convolutional layer
        local_features = self.concat_conv(concatenated_features)
